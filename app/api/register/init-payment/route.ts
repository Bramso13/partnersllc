...
export async function GET(request: NextRequest) {
  ...
  if (result.error) {
    if (result.error === "INVALID_LINK") {
      return NextResponse.redirect(
        new URL(`/register/${encodeURIComponent(token)}?error=INVALID_LINK`, BASE_URL),
        { status: 302 }
      );
    }
    return NextResponse.redirect(
      new URL(`/register/${encodeURIComponent(token)}?error=${encodeURIComponent(result.error)}`, BASE_URL),
      { status: 302 }
    );
  }
  ...
  if (!result.redirectUrl || !result.sessionId) {
    return NextResponse.redirect(
      new URL(`/register/${encodeURIComponent(token)}?error=FAILED_TO_CREATE_CHECKOUT_SESSION`, BASE_URL),
      { status: 302 }
    );
  }
