...
export default function ParametresPage() {
  ...
  const handleAccountSave = async () => {
    ...
    try {
      const fullName =
        `${accountForm.firstName.trim()} ${accountForm.lastName.trim()}`.trim();
      ...
      const errorMessage =
        error?.message || error?.code || "Une erreur inconnue s'est produite";
      setSaveMessage({
        type: "error",
        text: `Erreur lors de l'enregistrement: ${errorMessage}`,
      });
    } finally {
      setSaving(false);
    }
        ...
        <header className="sticky top-0 z-10 border-b border-neutral-800 bg-neutral-950/80 backdrop-blur-sm">
          <div className="flex items-center justify-between px-4 py-4 lg:px-8">
            <div className="flex items-center gap-4">
              ...
              <Logo variant="auth" brand="partnershub" />
            </div>
                        ...
                        if (user) {
                          try {
                            ...
                            if (profileData) {
                              ...
                              const nameParts = fullName.split(" ");
                              const phone =
                                (profileData as any).telephone || "";

                              // Recharger la bio depuis partners_hub_profiles
                              const { data: partnersHubProfile } =
                                await supabase
                                  .from("partners_hub_profiles")
                                  .select("about_you")
                                  .eq("user_id", user.id)
                  ...
                  {[
                    {
                      ...
                      desc: "Recevez des emails pour les mises à jour importantes",
                    },
                    {
                      key: "push",
                      label: "Notifications push",
                      desc: "Recevez des notifications sur votre navigateur",
                    },
                    {
                      key: "newEvents",
                      label: "Nouveaux événements",
                      desc: "Soyez alerté des nouveaux événements disponibles",
                    },
                    {
                      key: "privateMessages",
                      label: "Messages privés",
                      desc: "Recevez une notification pour chaque nouveau message",
                    },
                    {
                      key: "groupActivity",
                      label: "Activité des groupes",
                      desc: "Notifications pour les groupes que vous avez rejoints",
                    },
                    {
                      key: "weeklyNewsletter",
                      label: "Newsletter hebdomadaire",
                      desc: "Recevez des activités de la semaine",
                    },
                  ].map((notif) => (
                    <div
                      key={notif.key}
                  ...
                  {[
                    {
                      key: "publicProfile",
                      label: "Profil public",
                      desc: "Votre profil est visible par tous les membres",
                    },
                    {
                      key: "showOnlineStatus",
                      label: "Afficher mon statut en ligne",
                      desc: "Les autres membres peuvent voir si vous êtes en ligne",
                    },
                    {
                      key: "showEmail",
                      label: "Afficher mon email",
                      desc: "Votre email est visible sur votre profil public",
                    },
                    {
                      key: "appearInSearch",
                      label: "Apparaître dans les recherches",
                      desc: "Les membres peuvent vous trouver via la recherche",
                    },
                  ].map((priv) => (
                    <div
                      key={priv.key}
                      ...
                      <button
                        onClick={() =>
                          setPrivacy({
                            ...privacy,
                            [priv.key]:
                              !privacy[priv.key as keyof typeof privacy],
                          })
                        }
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          privacy[priv.key as keyof typeof privacy]
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setPrivacy({
                          ...privacy,
                          whoCanSeeConnections: e.target.value,
                        })
                      }
                      className="w-full rounded-lg border border-neutral-700 bg-neutral-900 px-4 py-3 text-white focus:border-green-500 focus:outline-none focus:ring-1 focus:ring-green-500"
                    >
