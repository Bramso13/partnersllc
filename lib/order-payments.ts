...
/**
 * Récupère tous les paiements d'une commande, triés par paid_at.
 */
...
/**
 * Calcule la somme des montants payés pour une commande (order_payments.amount).
 */
export async function getPaymentsSumForOrderId(
  orderId: string
): Promise<number> {
  const supabase = createAdminClient();
  const { data, error } = await supabase
...
/**
 * Pour plusieurs commandes, retourne un map order_id -> somme des paiements.
 */
export async function getPaymentsSumByOrderIds(
  orderIds: string[]
...
/**
 * Ajoute un paiement manuel à une commande.
 * Si la somme des paiements >= order.amount, met à jour order.status = PAID et order.paid_at.
 * Utilisé par l'API POST /api/admin/orders/[id]/payments et par les tests.
 */
